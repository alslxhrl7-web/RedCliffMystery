    ...
    public static class DialogueDataBootstrapper
    {
        ...
        public static void GenerateAll()
        {
            ...
            Debug.Log("[DialogueDataBootstrapper] 대사/엔딩 데이터 생성 완료: " + OutputFolder + ", " + EndingOutputFolder +
                       "\n※ 씬을 직접 구성한다면, 새로 생성된 엔딩 7종(true_ending, normal_ending, bad_default, " +
                       "bad_dusuk_document_lost, bad_caimao_* 3개)을 EndingManager의 '엔딩 목록'에 이 순서(트루 → 노멀 → " +
        ...
        private static void CreateFinalAccusationDialogueSet()
        {
            ...
            // 설평 정답 지목 — 노멀 엔딩: 범인은 잡았으나 화공 대비 실패로 적벽에서 패배
            d.normalEndingLines = new List<DialogueLine>
            {
                L("나레이션", "[최종 심문]"),
                L("조조", "그래서 범인은 누구라고 생각하느냐?"),
                L(null, "설평입니다."),
                L("조조", "……설평?"),
                L(null, "문서고를 자유롭게 드나들 수 있었고, 사건이 발생한 밤 그의 행동에도 의심스러운 점이 있었습니다."),
                L("조조", "그것만으로 범인이라 확신할 수 있느냐?"),
                L(null, "제가 찾아낸 증거들이 그를 가리키고 있습니다."),
                L("나레이션", "조조가 잠시 증거를 살펴본다."),
                L("조조", "설평을 데려와라."),
                L("나레이션", "[설평 심문]"),
                L("나레이션", "병사들이 설평을 데리고 들어온다."),
                L("조조", "설평."),
                L("설평", "예, 대장군."),
                L("조조", "사라진 문서에 대해 알고 있느냐?"),
                L("설평", "모릅니다."),
                L(null, "거짓말하지 마라."),
                L(null, "네가 문서고에 접근할 수 있었다는 사실은 확인됐다."),
                L("설평", "……."),
                L("조조", "대답하라."),
                L("설평", "……."),
                L("나레이션", "설평이 끝내 입을 다문다."),
                L("조조", "끌고 가라."),
                L("설평", "……."),
                L("나레이션", "병사들이 설평을 끌고 나간다."),
                L("나레이션", "[며칠 후 — 적벽]"),
                L("나레이션", "전장의 함성."),
                L("나레이션", "조조의 군선들이 강 위에 늘어서 있다."),
                L("조조", "화살을 준비하라!"),
                L("병사", "예!"),
                L("나레이션", "잠시 후, 강 건너에서 불길이 번지기 시작한다."),
                L("병사", "대장군! 불입니다!"),
                L("조조", "……뭐라고?"),
                L("병사", "적군의 화공입니다!"),
                L("나레이션", "불길이 연이어 군선으로 옮겨붙는다."),
                L("조조", "닻을 올려라! 배를 움직여라!"),
                L("병사", "배들이 서로 묶여 움직이지 않습니다!"),
                L("나레이션", "혼란이 시작된다."),
                L("조조", "……."),
                L("나레이션", "플레이어가 불타는 군선을 바라본다."),
                L(null, "설마……."),
                L(null, "이것까지는 막지 못한 건가……."),
                L("나레이션", "[패배 이후]"),
                L("나레이션", "불타버린 군영."),
                L("나레이션", "플레이어와 조조가 강을 바라보고 있다."),
                L("조조", "범인은 잡았다."),
                L("조조", "하지만 이미 늦었다."),
                L(null, "……."),
                L("조조", "문서를 훔친 자를 찾아냈다고 해서 전쟁의 결과까지 바뀌는 것은 아니었다."),
                L(null, "제가 조금만 더 빨리……."),
                L("조조", "아니다."),
                L("조조", "이미 벌어진 일이다."),
                L("나레이션", "조조가 불타버린 함대를 바라본다."),
                L("조조", "우리는 패했다."),
                L(null, "……."),
                L("조조", "적벽에서의 패배는 되돌릴 수 없다."),
                L("나레이션", "[마지막 장면]"),
                L("나레이션", "강 위에 떠 있는 잿더미."),
                L("나레이션", "플레이어가 조용히 바라본다."),
                L("나레이션", "\"범인은 찾아냈다.\""),
                L("나레이션", "\"사라진 문서의 사건은 끝났다.\""),
                L("나레이션", "\"하지만 진실을 밝혀내는 것만으로는 충분하지 않았다.\""),
                L("나레이션", "\"이미 전쟁은 시작되었고, 적벽에서의 패배는 돌이킬 수 없었다.\""),
                L("나레이션", "\"조금만 더 일찍 진실에 닿았다면…….\""),
                L("나레이션", "화면 암전."),
            };
        ...
        // ------------------------------------------------------------------
        // 메인 엔딩 3종 (기획서 6절 표) — 트루 / 노멀 / 기본(조건 없음) 배드
        // ------------------------------------------------------------------
        private static void CreateMainEndings()
        {
            ...
            CreateEndingAsset(
                ...
                EndingType.Normal,
                "노멀 엔딩 — 늦어버린 진실",
                "설평(목연)을 정확히 지목해 체포했지만, 화공에 대한 결정적인 대비책까지는 마련하지 못했다. " +
                "조조는 진범을 잡았음에도 이미 시작된 전쟁의 흐름을 막지 못했고, 역사대로 적벽에서 패배했다.\n\n" +
                "\"범인은 찾아냈다. 그러나 진실을 밝혀내는 것만으로는 충분하지 않았다. 이미 전쟁은 시작되었고, " +
                "적벽에서의 패배는 돌이킬 수 없었다.\"",
                new List<EndingCondition>
                {
                    new EndingCondition { type = EndingCondition.ConditionType.FlagIsTrue, key = "final_accusation_correct" },
