...
namespace RedCliffMystery.Dialogue
{
    ...
    /// 3) 방통/장간/두석/설평을 고르면 기존처럼 confirmButton으로 바로 확정합니다.
    /// 4) 채모·장윤을 고르면 지금까지 수집한 단서 중 "증거로 제시할 것"을 체크박스로 고르게 하고,
    ///    제시한 개수에 따라 세 가지 배드 엔딩 컷씬(무죄 추정 없이 지목 / 정황을 증거로 착각 / 처형 후에도
    ///    문서 미발견) 중 하나가 재생된 뒤 엔딩으로 넘어갑니다. 채모·장윤은 어떤 경우에도 진범이 아니므로
    ///    이 갈래는 항상 배드 엔딩으로 귀결되고, 설평이 진범이라는 사실은 어느 경우에도 공개하지 않습니다.
    /// </summary>
